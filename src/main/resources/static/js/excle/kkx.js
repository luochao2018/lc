        ...
        $.each(v.condition, function (i, j) {
            ...
            if (id.ftable == "table-search2") {
                if (i == 0 || i % (id.table.queryNum) == 0) {
                    tr += i;
                    $("#" + id.ftable).append("<tr class='" + tr + "'><th style='" + th + "'>" + j.title + ":</th><td style='" + td + "'>" + condition_type.input.replace(/@@COLUMN/g, j.name) + "</td></tr>");
                } else {
                    $("." + tr).append("<th style='" + th + "'>" + j.title + ":</th><td style='" + td + "'>" + condition_type.input.replace(/@@COLUMN/g, j.name) + "</td>");
                }
            } else {
                $("#" + id.ftable).append("<tr><th class='th'>" + j.title + ":</th><td class='td'>" + condition_type.input.replace(/@@COLUMN/g, j.name) + "</td></tr>");
            }
            //初始化输入框
...
function initDataGrid(v) {
    ...
    var datagrid = $("#" + datagridID).datagrid({
        ...
        onClickRow: function (index, row) {
            ...
            DATAGRID_ID = DATAGRID_ID && DATAGRID_ID != datagridID ? DATAGRID_ID : sid[0];//如果没有绑定元素,绑定第一个子id
            if (!DATAGRID_ID) {
                return;
            }
            var formPkid = 0;
        ...
        onRowContextMenu: function (e, rowIndex, rowData) { //右键时触发事件
            if (rowData == null || $.isEmptyObject(v.menu)) {
                return;
            }
            initMenu({"table": v.table, "menu": v.menu, "row": rowData, "formT": v.formT});
        ...
        onDblClickRow: function (rowIndex, rowData) {
            ...
            //     editRow = rowIndex;
            // }
            initForm({table: v.table, ftable: "table-dg"}, v.formT);
            var o = {
        ...
        toolbar: [{
            ...
            }, '-',
            {
                ...
                handler: function () {
                    initForm({table: v.table, ftable: "export"}, []);
            ...
            }, '-',
            {
                ...
                handler: function () {
                    initForm({table: v.table, ftable: "import"}, []);
            ...
            }, '-',
            {
                ...
                handler: function () {
                    initForm({table: v.table, ftable: "table-search"}, v.condition);
    ...
    if (v.table.firstload == 1) {
        $("#" + datagridID).datagrid("options").queryParams = $.extend(
            {},
            $("#" + datagridID).datagrid("options").queryParams,
            {FORM_PKID: v.table.pkid, FORM_TABLE: v.table.table}
        );
        reload_(datagridID);
    }
}
