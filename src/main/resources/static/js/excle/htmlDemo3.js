 ...
 */
$(function () {
    getIformation(getQueryString("formid"));
});
 ...
 * @param v
 */
function getIformation(formid) {

    var obj = {
        url: ACTION_URL + "getIformation",
        param: {
            formid: formid,
        }
    };
        ...
        if (!$.isEmptyObject(info.table.SID)) {
            $.each(info.table.SID, function (index, value) {
                if (value) {
                    ...
                    load_(value, $("#" + $.extend({}, info.DIALOG.FORM).getForm(), {
        ...
        if (!value.DISABLED) {//不禁用时才重定义onclick
            ...
            if (value.ID == "m-drillthrough") {
                $("#m-drillthrough").click(function () {
                    PAGE.param = info.row;
                    var windows = window.open(ACTION_URL + value.HREF);
                });
            }
        }
    });
...
function initColumn_(info) {
    ...
    var pageSize = parseInt((height - 100 - 32.2 * count) / 32.2);//减去datagrid的toolbar/headtitle/pager,再往上空两行
